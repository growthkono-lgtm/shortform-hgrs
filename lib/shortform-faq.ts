 ...
 * /shortform 자주 묻는 질문.
 *
 * 컴포넌트 안에 갇혀 있던 것을 2026-08-20 여기로 옮겼다. 화면에는 7문항이
 * 그대로 나오는데 `FAQPage` 구조화 데이터가 안 붙어 있었다 — 생성형 검색과
 * 구글 FAQ 리치리절트가 이 문답을 인용할 길이 없었다는 뜻이다.
 * /sns-brand 가 `lib/sns-brand.ts` 의 FAQ 를 쓰는 것과 같은 모양으로 맞춘다.
 *
 * 문안은 사장님이 확정한 그대로다. 여기 없는 문항은 넣지 않는다.
 */
export const SHORTFORM_FAQ: { q: string; a: string }[] = [
  {
