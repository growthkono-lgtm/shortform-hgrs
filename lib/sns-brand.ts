/**
 * /sns-brand — "브랜드 채널 마케팅" **리드 확보 랜딩**의 카피 단일 출처.
 *
 * 2026-08-20 개명. 갈라져 있던 두 서비스(브랜드 SNS 채널 커뮤니케이션 +
 * 종합 브랜드 마케팅)를 소개서(`scripts/build-deck.ts`)에서 **하나로 합치고**
 * 이름을 "브랜드 채널 마케팅 프로젝트" 로 정했다. 홈페이지도 같은 이름을 쓴다.
 * URL(`/sns-brand`)은 그대로다 — 외부 링크가 이미 나가 있다.
 *
 * 규칙 하나만 지키면 된다:
...
/**
 * 히어로 — 소개서 PART2 표지(`build-deck.ts` `part-brand`)와 같은 문안이다.
 * `sub`·`stats` 는 소개서가 이 상수를 그대로 읽어 간다. 고치면 양쪽이 같이 바뀐다.
 */
export const HERO = {
...
/**
 * 브랜드 채널 마케팅 프로젝트 — 두 플랜. (2026-08-20 사장님 구술)
 *
 * 소개서(`scripts/build-deck.ts` `brand-plan` 장)에서 확정한 문안을 **한 글자도
 * 바꾸지 않고** 옮겼다. 소개서와 홈페이지가 다른 말을 하면 통화가 어긋난다.
 *
 * ⚠️ 두 가지를 지킬 것 —
 *   1. **금액 숫자를 만들어 넣지 않는다.** 확인된 것은 "협의" 와 "월 3곳 한정",
 *      "6개월 또는 1년" 뿐이다. 편수 단위 정가가 있는 라인은 숏폼뿐이다.
 *   2. **PLAN A 에 투입 조직을 적지 않는다. 광고·CRM 도 여기서 말하지 않는다.**
 *      사장님: *"거기에 투입조직은 넣지마, 광고 crm은 말안할거야 여기서."*
 *      그 둘은 PLAN B 의 이야기다.
 */
export const BRAND_PLANS = {
  eyebrow: "Plans",
  title: ["브랜드 채널 마케팅 프로젝트"],
  lead: "두 갈래로 나눠 진행합니다 — 필요한 쪽만 골라도 됩니다.",
  plans: [
    {
      key: "PLAN A",
      name: "SNS 채널 활성화 플랜",
      ...
      items: [
        { title: "유튜브 채널 운영", note: "+ 쇼츠 · 릴스 미러링" },
        { title: "SEO / AEO", note: "+ 브랜드 블로그 최적화" },
      ],
      meta: [
        { k: "계약 단위", v: "6개월 또는 1년" },
        { k: "금액", v: "브랜드 상황에 따라 협의" },
      ],
    },
    {
      key: "PLAN B",
      ...
      items: [
        {
          title: "퍼포먼스 마케팅",
          note: "메타 · 구글 메인 + PMF 채널 · 버티컬 · 네트워크 조합",
        },
        { title: "CRM 캠페인 최적화", note: "멤버십 설계" },
        { title: "데이터 · 그로스 파이프라인 구축", note: "" },
      ],
      meta: [
        { k: "진행 한도", v: "월 3곳 한정" },
        { k: "금액", v: "브랜드 단위 협의" },
  ...
  note: "편수 단위 정가 결제는 숏폼 스튜디오뿐입니다 · 부가세 별도",
} as const;
